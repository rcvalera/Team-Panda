...
function myFunction() {
    ...
    var famnameValue = document.getElementById("flyname").value;
    ...
    var alphaNum = /^(?:[0-9]+[a-z]|[a-z]+[0-9])[a-z0-9]*$/i;
       ...
       if (famnameValue == "") {
        setErrorFor("flyname","flyname-tooltip", 'Family Name cannot be blank');
          return false;

       } else {

          if(letters.test(famNameValue)){
            setSuccessFor("flyname", "flyname-tooltip");

        }

        else {
            setErrorFor("flyname", "flyname-tooltip", 'Alphabets only!');
